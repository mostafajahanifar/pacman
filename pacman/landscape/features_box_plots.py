...
for feat in feats_list:
    ...
    fig, (ax1, ax3, ax4, ax2) = plt.subplots(
        1, 4, figsize=(14, 3), gridspec_kw={"width_ratios": [30, 2, 5, 2]}
    )
    ...
    sns.boxplot(
        ax=ax3, y=feat, x="gender", data=df, showfliers=False, palette=gender_palette
    )
    ax3.set_ylabel("")
    ax3.set_xlabel("")
    ...
    hot_cold_palette = {"Hot": cmap(0.999), "Cold": cmap(0)}
    sns.boxplot(
        ax=ax2,
        y=feat,
        x="temperature",
    ...
    ax2.set_xticklabels(
        ax2.get_xticklabels(), rotation=90, ha="center"
    ...
    plt.savefig(
        f"{save_dir}/boxplot_{feat}.png", dpi=600, bbox_inches="tight", pad_inches=0.01
    )
