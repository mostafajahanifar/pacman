...
type_temp_counts = mitosis_feats.groupby(['type', 'temperature']).size().unstack(fill_value=0)
...
# ---------- Gender Distribution ----------
gender_temp_counts = mitosis_feats.groupby(['gender', 'temperature']).size().unstack(fill_value=0)
...
race_temp_counts = mitosis_feats.groupby(['race', 'temperature']).size().unstack(fill_value=0)
...
plt.savefig(f"{save_dir}/hot-cold-distribution.png", dpi=600, bbox_inches='tight', pad_inches=0.01)