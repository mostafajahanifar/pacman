...
print(7 * "=" * 7)
print("Ploting mitotic activity distribution over different ethnicities")
print(7 * "=" * 7)
...
for feature in features_list:
    ...
    for c_type in mitosis_feats["type"].unique():
        if (
            len(
                mitosis_feats.loc[
                    (mitosis_feats["type"] == c_type)
                    & (mitosis_feats["race"] == "White")
                ]
            )
            > 0
            and len(
    ...
    plt.savefig(
        f"{save_dir}/ethnicities_dist_{feature}.png",
        dpi=600,
        bbox_inches="tight",
        pad_inches=0.01,
    )
