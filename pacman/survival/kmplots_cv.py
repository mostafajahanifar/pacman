...
feats_list = "mean(ND)" # if a list, will use CoxPH model, if not, raw feature values are used.
...
def cv_helper(discov_df, feats_list, event_col, time_col, split_folder, km_plot=False, x_label="Months", y_label=None, add_counts=False, shuffle_data=False, cutoff_mode="median"):
    ...
    for run in ex_iterator:
        ...
        output = survival_stratification_analysis(train_set, test_set, feats_list, time_col, event_col, censor_at, cutoff_mode, cutoff_point)
    ...
    if km_plot:
        ...
        ax = km_upper.fit(T_upper_test, event_observed=E_upper_test, label=upper_label).plot_survival_function(ax=ax, show_censors=True, censor_styles={'ms': 4}, color='r', ci_show=False, xlabel=x_label, ylabel=y_label)
        ax = km_lower.fit(T_lower_test, event_observed=E_lower_test, label=lower_label).plot_survival_function(ax=ax, show_censors=True, censor_styles={'ms': 4}, color='b', ci_show=False, xlabel=x_label, ylabel=y_label)
        ...
        if add_counts:
            ...
            # Initializing the KaplanMeierModel for each group
            ax_copy = km_upper.fit(T_upper_test, event_observed=E_upper_test, label=upper_label).plot_survival_function(ax=ax_copy, show_censors=True, censor_styles={'ms': 5}, color='r', ci_show=False, xlabel=x_label, ylabel=y_label)
            ax_copy = km_lower.fit(T_lower_test, event_observed=E_lower_test, label=lower_label).plot_survival_function(ax=ax_copy, show_censors=True, censor_styles={'ms': 5}, color='b', ci_show=False, xlabel=x_label, ylabel=y_label)

            add_at_risk_counts(km_lower, km_upper, rows_to_show=['At risk'], xticks=[0, 25, 50, 75, 100], ypos=-.6,
                                 colors=["blue", "red"],
    ...
    for event_type in ["DSS", "PFI", "OS", "DFI"]:
        ...
        for study in items:
            ...
            try:
                ...
                df_to_save, logrank_results, km_fig, km_fig_counts = cv_helper(discov_df, feats_list,  event_col, time_col, split_folder, km_plot=True, add_counts=True, cutoff_mode=cutoff_mode)
                ...
                for _ in tqdm(range(PERM_REPS), desc='p-value permutation tests'):
                    try:
                        _, logrank_results_repeats = cv_helper(discov_df, feats_list, event_col, time_col, split_folder, km_plot=False, shuffle_data=True, cutoff_mode=cutoff_mode)
                    except:
                        continue
                ...
                # save the results
                save_path = save_dir + f"kmplot_cv_{study}_{event_type}_censor{censor_at}_cindex{avr_cindex:.2}_pvalue{corrected_p_value:.3}"
                ...
                # save the results
                save_path = save_dir + f"risked_kmplot_cv_{study}_{event_type}_censor{censor_at}_cindex{avr_cindex:.2}_pvalue{corrected_p_value:.3}"
                km_fig_counts.savefig(save_path + '.png', dpi=600, bbox_inches = 'tight', pad_inches = 0.01)
            except Exception as e:
                print(f"! Failed working on {study} study in {event_type} endpoint: {e}")