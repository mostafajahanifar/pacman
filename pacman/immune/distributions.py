...
parser = argparse.ArgumentParser(
    description="Immune Subtypes Distribution Comparison Analysis"
)
parser.add_argument(
    "--mode",
    type=str,
    default="Pan-cancer",
    choices=["pan-cancer", "all"],
...
mitosis_feats = mitosis_feats[
    ["bcr_patient_barcode", "type", "temperature"] + selected_feats
]
...
for cancer_type in items:
    ...
    working_df = pd.concat(
        [df1_common[selected_feats], df2_common["Immune Subtype"]], axis=1
    )
    ...
    for mit_feat in selected_feats:
        try:
            ...
            annotator = Annotator(
                ax, significant_pairs, data=working_df, x="Immune Subtype", y=mit_feat
            )
            annotator.configure(
                test="Mann-Whitney",
                text_format="star",
            ...
            plt.savefig(
                f"{save_dir}/mitosis_{mit_feat}_in_immunue_subtypes.png",
                dpi=600,
                bbox_inches="tight",
                pad_inches=0.01,
            )
            ...
            print(
                f"Error BOX plotting {mit_feat} in immune subtypes for {cancer_type}: {e}"
            )

    # plot hot/cold population in immune subtypes
    try:
        working_df = pd.concat(
            [df1_common["temperature"], df2_common["Immune Subtype"]], axis=1
        )
        ...
        grouped = (
            working_df.groupby(["Immune Subtype", "Mitotic Temperature"])
            .size()
        ...
        grouped["Ratio"] = grouped.apply(
            lambda row: simplify_ratio(row.get("Cold", 0), row.get("Hot", 0)), axis=1
        )
        # Add the calculated approximate ratio on top of each bar group
        for idx, (immune_subtype, row) in enumerate(grouped.iterrows()):
            ratio = row["Ratio"]
            max_height = (
                max(row["Hot"], row["Cold"]) if "Hot" in row and "Cold" in row else 0
            )
        ...
        plt.savefig(
            f"{save_dir}/population_Hot-Cold_in_immunue_subtypes.png",
            dpi=600,
            bbox_inches="tight",
            pad_inches=0.01,
        )
        ...
        print(
            f"Error plotting population of Hot-Cold in immune subtypes for {cancer_type}: {e}"
        )
    ...
    for immune_feat in immune_feats:
        try:
            working_df = pd.concat(
                [df1_common["temperature"], df2_common[immune_feat]], axis=1
            )
            working_df = working_df.dropna(axis=0, how="any")
            working_df = working_df.rename(
                columns={"temperature": "Mitotic Temperature"}
            )
            ...
            if immune_feat in [
                ...
                working_df = working_df.rename(
                    columns={immune_feat: f"-log10({immune_feat}+1)"}
                )
            ...
            plt.tight_layout()
            plt.savefig(
                f"{save_dir}/immune_{immune_feat}_in_Hot-Cold.png",
                dpi=600,
                bbox_inches="tight",
                pad_inches=0.01,
            )
            ...
            print(
                f"Error VIOLIN plotting immune feature {immune_feat} in {cancer_type}: {e}"
            )
