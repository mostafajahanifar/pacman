...
for cancer_type in  SURV_CANCERS + ["Pan-cancer"]: # ALL_CANCERS +
    ...
    for event_type in ["OS", "PFI", "DSS", "DFI"]:
        ...
        for temp in ["Hot", "Cold"]:
            ...
            try:
                cutoff = median_cut_off(working_df[selected_feats[0]].values.reshape(-1, 1), )
            except:
                continue
            ...
            working_df["labeled_cluster"] = working_df[selected_feats[0]].apply(lambda x: f"M{temp[0]}AH" if x > cutoff else f"M{temp[0]}AL")
            ...
            ax.set_ylim([0, 1])
            plt.tight_layout()
            plt.savefig(f"{save_dir}/{temp}_km_{event_type}_{feat_name}_censor{censor_at}.png", dpi=600, bbox_inches = 'tight', pad_inches = 0.01)
           ...
           # add the risk counts and plot
            ...
            add_at_risk_counts(*kmf_fits, rows_to_show=['At risk'], xticks=[0, 25, 50, 75, 100], ypos=-0.4,
                                colors=[cluster_colors[cluster] for cluster in clusters], ax=ax)

            plt.savefig(f"{save_dir}/risked_{temp}_km_{event_type}_{feat_name}_censor{censor_at}.png", dpi=600, bbox_inches = 'tight', pad_inches = 0.01)
