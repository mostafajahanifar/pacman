    ...
    for event_type in event_types:
        ...
        for cancer in cancer_types:
            ...
            for feat in selected_feats:
                try:
                    hr, ci_lower, ci_upper, p_value = cox_univariate_hr_analysis(
                        cancer_df, feat, event_col, time_col
                    )
    ...
    for event_type, cancers in results.items():
        ...
        if mode == "selected":
            cancer_types = [
                ...
                for cancer in cancer_types
                if any(
                    cancers[cancer][feat] and cancers[cancer][feat][3] <= 0.05
                    for feat in selected_feats
                )
            ]
        ...
        fig, axs = plt.subplots(
            num_cancers,
            num_feats,
            figsize=(num_feats * 2, num_cancers / 3.5),
            sharex=True,
            sharey=True,
        )
        ...
        for i, cancer in enumerate(cancer_types):
            for j, feat in enumerate(selected_feats):
                ...
                ax.axvline(
                    x=1, color="lightgray", linestyle="--", linewidth=1, zorder=0
                )  # Line at HR=1
                ax.set_xlim([0, 3])
                ax.set_xticks([1, 2])
        ...
        plt.savefig(
            f"{save_root}/hr_{mode}_{mit_temp}_{event_type}.png",
            dpi=600,
            bbox_inches="tight",
            pad_inches=0.01,
        )
...
univariate_hr_results = analyze_univariate_hr_by_event_and_cancer(
    mitosis_feats, selected_feats
)
