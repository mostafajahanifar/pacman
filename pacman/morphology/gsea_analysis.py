...
mit_signatures = (
    pd.read_csv(os.path.join(DATA_DIR, "signatures.csv"))["Mitosis Process"]
    .dropna()
    .to_list()
)
selected_feats = [
...
for cancer_type in items:
    counts_org = pd.read_csv(
        os.path.join(DATA_DIR, f"raw_gene_counts/{cancer_type}_gene_raw_counts.csv")
    )
    ...
    try:
        ...
        if temp in ["Cold", "Hot"]:
            cancer_mitosis_feats = cancer_mitosis_feats_all[
                cancer_mitosis_feats_all["temperature"] == temp
            ]
        else:
        ...
        common_cases = pd.Series(
            list(
                set(cancer_mitosis_feats["bcr_patient_barcode"]).intersection(
                    set(counts_org["Case ID"])
                )
            )
        )
        ## Keep only the rows with the common case names in both dataframes
        df1_common = cancer_mitosis_feats[
            cancer_mitosis_feats["bcr_patient_barcode"].isin(common_cases)
        ]
        ...
        if np.isnan(feat_med):
            ...
            continue
        df1_common["atypical"] = df1_common[selected_feats[0]].apply(
            lambda x: "High" if x > feat_med else "Low"
        )
        ...
        metadata = metadata.rename(
            columns={"bcr_patient_barcode": "Sample", "atypical": "Condition"}
        )
        ...
        # Get DESeq stats
        stat_res = DeseqStats(dds, contrast=("Condition", "High", "Low"), alpha=0.01)
        stat_res.summary()
        ...
        df_sorted = sigs[
            (sigs["padj"] < pval_thresh)
            & (sigs["log2FoldChange"].abs() > log2fc_thresh)
        ]
        ...
        if len(mitosis_rows) > 0:
            if len(mitosis_rows) > 6:
                top_m_rows = (
                    mitosis_rows[mitosis_rows["sorter"] > 0]["Symbol"].head(3).to_list()
                )
                bottom_m_rows = (
                    mitosis_rows[mitosis_rows["sorter"] < 0]["Symbol"].head(3).to_list()
                )
