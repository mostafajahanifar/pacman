...
cin_df = pd.read_excel(
    os.path.join(DATA_DIR, "tcga_cin_signatures.xlsx"),
    sheet_name="ST_18_TCGA_Activities_raw",
)
...
for cancer_type in ALL_CANCERS + ["Mitotic Hot", "Mitotic Cold", "Pan-cancer"]:
    ...
    if cancer_type == "Mitotic Hot":
        mitosis_feats_cancer = mitosis_feats_cancer[
            mitosis_feats_cancer["temperature"] == "Hot"
        ]
    if cancer_type == "Mitotic Cold":
        mitosis_feats_cancer = mitosis_feats_cancer[
            mitosis_feats_cancer["temperature"] == "Cold"
        ]
    ...
    common_cases = pd.Series(
        list(
            set(mitosis_feats_cancer["bcr_patient_barcode"]).intersection(
                set(gene_exp_cancer["TCGA Participant Barcode"])
            )
        )
    )
    ...
    df2_common = gene_exp_cancer[
        gene_exp_cancer["TCGA Participant Barcode"].isin(common_cases)
    ]
    ...
    corr_matrix, pvalue_matrix = calculate_corr_matrix(
        Y, X, method="spearman", pvalue_correction="fdr_bh"
    )
...
fig, (ax1, ax2, ax3) = plt.subplots(
    ...
    3,
    figsize=(6, 4.4),
    gridspec_kw={"width_ratios": [corr_matrix.shape[1] - 3, 2, 1]},
)
...
heatmap1 = sns.heatmap(
    ...
    linecolor="gray",
    square=True,
    annot=annotations.iloc[:, :-3],
    fmt="",
    annot_kws={"size": 8, "va": "center_baseline", "ha": "center"},
    yticklabels=True,
    xticklabels=True,
    ax=ax1,
)
...
plt.savefig(
    f"{save_root}/cin_correlations_{atyp_feat}_selected_spearmanFDR.png",
    dpi=600,
    bbox_inches="tight",
    pad_inches=0.01,
)
corr_matrix.to_csv(
    f"{save_root}/cin_correlations_{atyp_feat}_selected_spearmanFDR_corr.csv"
)
pvalue_matrix.to_csv(
    f"{save_root}/cin_correlations_{atyp_feat}_selected_spearmanFDR_pval.csv"
)
