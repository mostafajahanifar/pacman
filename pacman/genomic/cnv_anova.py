...
def calculate_annova_matrix(X, Y):
    """
    Compute a matrix of one-way ANOVA F-statistics and p-values for associations
    between each pair of columns in X and Y.

    Parameters:
        X (pd.DataFrame): DataFrame where each column represents a continuous phenotype.
        Y (pd.DataFrame): DataFrame where each column represents a categorical variable
            (e.g., CNV levels).

    Returns:
        f_matrix (pd.DataFrame): Matrix of F-statistics with shape
            (X.columns × Y.columns).
        p_matrix (pd.DataFrame): Matrix of corresponding p-values with shape
            (X.columns × Y.columns).
    """
...
for ci, cancer_type in enumerate(sorted(gene_cnv_all["type"].unique())):
    ...
    plt.savefig(
        save_dir + f"cnv_anova_{cancer_type}_top20.pdf",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0,
    )
    ...
    auc_matrix_reordered = auc_matrix_reordered[
        ["mean(HC)", "mean(CL)", "cv(ND)", "mean(ND)", "HSC"]
    ]
    ...
    fig, (ax_violin, ax_dotplot) = plt.subplots(
        2,
        1,
    ...
    Y_selected = Y[auc_matrix_reordered.index].melt(
        var_name="Variable", value_name="CNV"
    )
    ...
    sizes = (dot_data["size"] - pval_min) / (pval_max - pval_min) * (
        size_max - size_min
    ) + size_min
    ...
    scatter = ax_dotplot.scatter(
        ...
        y=dot_data["y"],
        c=colors,
        s=sizes,  # Adjust size scaling as needed
        edgecolor="lightgray",
        linewidth=0.5,
    )
    ...
    ax_dotplot.set_xlim(-0.5, len(auc_matrix_reordered.index) - 0.5)
    if ci != 0:
    ...
    plt.tight_layout()
    fig.savefig(
        save_dir + f"cnv_anova_{cancer_type}_top5.pdf",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0.01,
        transparent=True,
    ...
    fig.savefig(
        save_dir + f"cnv_anova_{cancer_type}_top5.png",
        dpi=600,
        bbox_inches="tight",
        pad_inches=0.01,
        transparent=True,
    ...
    fig, axes = plt.subplots(
        nrows=len(unique_x), ncols=len(unique_y), figsize=(fig_width, fig_height)
    )
    ...
    for x in unique_x:
        for y in unique_y:
            ...
            if not dot_data[
                (dot_data["x"] == x) & (dot_data["y"] == y) & (dot_data["pval"] < 0.01)
            ].empty:
                significant = True
            else:
                significant = False
            ...
            tukey.plot_simultaneous(
                ax=ax, figsize=(fig_width, fig_height), comparison_name=0.0, xlabel=y
            )
    ...
    fig.tight_layout()
    fig.savefig(
        save_dir + f"tuckey_{cancer_type}_plots.pdf",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0.01,
    )
