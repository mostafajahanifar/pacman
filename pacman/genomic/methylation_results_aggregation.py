...
def load_gene_name_mapping(mapping_file):
    ...
    df["NAME"] = df.apply(
        lambda row: row["ENTITY_STABLE_ID"] if pd.isna(row["NAME"]) else row["NAME"],
        axis=1,
    )
...
def process_correlation_cancer_type(
    cancer_type, corr_file, pval_file, writer, id_to_name
):
...
for cancer_type in sorted(os.listdir(base_dir)):
    ...
    if os.path.isdir(cancer_dir):
        ...
        if os.path.exists(corr_file) and os.path.exists(pval_file):
            process_correlation_cancer_type(
                cancer_type, corr_file, pval_file, writer, id_to_name
            )
