...
SUCCESS_RATE = 100
censor_at = -1
cv_experiment = 'FM_bootstrap_results/CV_results_NoCensor_median'
...
for alg in ['ml', 'cph']:
    ...
    for event_type in event_types:
        ...
        for cancer_type in cancer_types:
            ...
            directory = f"{cv_experiment}/FM_{cancer_type}/" # cv_results_{cancer_type}_{event_type}_censor{censor_at}_*.csv"
            file = f"FM_bootstrap_results_{cancer_type}_{event_type}_censor{censor_at}.csv"
            try:
            ...
            results_dict['pvalue'].append(csv_df[alg+'_pvalue'].median())
            results_dict['cindex_mean'].append(csv_df[alg+'_cindex'].mean())
            results_dict['cindex_std'].append(csv_df[alg+'_cindex'].std())

            if len(csv_df) < SUCCESS_RATE:
                results_dict['success'].append(0)
            else:
    ...
    formatted_df.to_excel(f'{cv_experiment}/FM_bootstrap_summary_{alg}.xlsx')
